      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            {/* Direct contact — one button per channel. We only expose
                channels we actually have data for (phone), so the
                sender doesn't get a greyed-out WhatsApp button when
                the transporter's number isn't on file. In-app chat
                is always offered as a privacy-preserving fallback. */}
            <div className="bg-white rounded-2xl border border-dz-gray-200 p-6 space-y-3">
              <h3 className="font-semibold text-dz-gray-800 mb-1 text-sm">
                Contacter {isDemande ? "l'expéditeur" : "le transporteur"}
              </h3>
              <p className="text-xs text-dz-gray-500 mb-2">
                Choisissez le canal qui vous convient. Toute conversation reste couverte par la protection Waselli tant que le paiement transite par la plateforme.
              </p>
                ...
                <>
                  <a
                    href={`https://wa.me/${phone.wa}?text=${encodeURIComponent(
                      `Bonjour ${transporter?.first_name ?? ""}, je vous contacte au sujet de l'annonce Waselli ${listing.from_city} → ${listing.to_city}.`
                    )}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20B954] text-white py-2.5 rounded-xl font-semibold text-sm transition-colors"
                  >
                  ...
                  <a
                    href={`tel:${phone.tel}`}
                    className="w-full flex items-center justify-center gap-2 bg-dz-gray-800 hover:bg-dz-gray-900 text-white py-2.5 rounded-xl font-semibold text-sm transition-colors"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.95.69l1.5 4.49a1 1 0 01-.27 1.04l-2.2 2.2a11 11 0 005.52 5.52l2.2-2.2a1 1 0 011.04-.27l4.49 1.5a1 1 0 01.69.95V19a2 2 0 01-2 2h-1C9.72 21 3 14.28 3 6V5z" />
                    </svg>
                    Appeler
                  </a>
                </>
              ...
              {/* In-app message — always available as a privacy-safe channel */}
              <div id="msg-box" className="pt-3 border-t border-dz-gray-100">
                <p className="text-xs font-medium text-dz-gray-600 mb-2">
                  {isDemande ? "Message personnalisé (optionnel)" : "Ou envoyez un message dans Waselli"}
                </p>
                ...
                <button onClick={handleSendMessage} disabled={sendingMsg}
                  className="w-full mt-2 border border-dz-green text-dz-green hover:bg-dz-green hover:text-white disabled:opacity-50 py-2.5 rounded-xl font-medium transition-colors text-sm">
                  {sendingMsg ? "Envoi..." : "Envoyer dans Waselli"}
                </button>
              </div>
            </div>
